 ...
 */
{
    ...
    FILE_PTR fp(fopen(filename, "r"), fclose);
    if (!fp)
    {
        debug_printf("Can't open table file %s\n", filename);
        return FALSE;
    }
    tblno = 0;

    while (read_text_integer(fp.get(), &val, &termchar))
    { /* read 1st element of table */
        if (tblno >= NUM_QUANT_TBLS)
        {
            debug_printf("Too many tables in file %s\n", filename);
            return FALSE;
        }
        table[0] = (unsigned int)val;
        for (i = 1; i < DCTSIZE2; i++)
        {
            if (!read_text_integer(fp.get(), &val, &termchar))
            {
                debug_printf("Invalid table data in file %s\n", filename);
                return FALSE;
            }
    ...
    if (termchar != EOF)
    {
        debug_printf("Non-numeric data in file %s\n", filename);
        return FALSE;
    }
 ...
 */
{
    ...
    for (ci = 0; ci < MAX_COMPONENTS; ci++)
    {
        if (*arg)
        {
            ...
            {
                debug_printf("JPEG quantization tables are numbered 0..%d\n",
                             NUM_QUANT_TBLS - 1);
                return FALSE;
            }
 ...
 */
{
    ...
    for (ci = 0; ci < MAX_COMPONENTS; ci++)
    {
        if (*arg)
        {
            ...
            {
                debug_printf("JPEG sampling factors must be 1..4\n");
                return FALSE;
            }
